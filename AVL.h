...
public:
	...
	string computeHash(string data) {
		...
		selectedValue = rowData[selectedColumns[0]];
		previousValue = rowData[selectedColumns[1]];
		nextValue = rowData[selectedColumns[2]];
	...
	string deleteHelper(string nodeFile, string key) {
		...
		while (getline(node, line)) {
			...
			if (line.find("hash=") == 0) hash = line.substr(5);
		}
		node.close();
	...
	string postOrderTraversalHash() {
		return postOrderTraversalHashHelper(rootFile);
	}
	string postOrderTraversalHashHelper(string filename) {
		if (filename == "null") return "";
		ifstream file(filename);
		if (!file.is_open()) {
			cerr << "Error: Could not open file " << filename << endl;
		...
		file.close();
		string leftHash = postOrderTraversalHashHelper(leftFile);
		string rightHash = postOrderTraversalHashHelper(rightFile);
		string combinedHash = leftHash + rightHash + hash;
